...
def color_map(ax2, gamma_ss, model, model_weights, Psi_model, cmaplist, terms, return_predictions=False):
    """
    ...
    for i in range(terms): # Iterate through all terms
        ...
        all_plots.append(ax2.plot(gamma_ss, 0*upper, lw=0.4, zorder=24, color='k'))
    if return_predictions:
        return all_plots, upper
...
def plot_arch_comp_graph( paths2saveResults, best_reg=2):
    ...
    cmaplist = [cmap(i) for i in range(cmap.N)]
    labels_ws = ["X", "Y", "Biax", "N/A", "N/A"]
    labels_45 = ["N/A", "N/A", "Biax", "N/A", "N/A"]
    labels_all = [labels_ws, labels_ws, labels_45]
    titles = ["w90", "s90", "x45"]
    ...
    for idx_path2saveResults in range(len(paths2saveResults)):
        ...
        ### Make bar chart of R^2 for particular
        # print("len(input_data", len(input_data), input_data)
...
def plot_training(dfs, Psi_model_type, modelFit_mode, p, path2saveResults, best_reg=2, is_I4beta=True):
    ...
    for i in range(3):
        # x_R2 = calculateR2(inputs[0][i][0], outputs[0][i][0], inputs[0][i][0], preds[0][i][0])
        # y_R2 = calculateR2(inputs[0][i][1], outputs[0][i][1], inputs[0][i][1], preds[0][i][1])
    ...
    ax_w.set_xlabel("X stretch [-]", fontweight="bold")
    ax_w.set_ylabel("X stress [kPa]", fontweight="bold")
    ax_s.set_xlabel("Y stretch[-]", fontweight="bold")
    ax_s.set_ylabel("Y stress [kPa]", fontweight="bold")
    ...
    plt.rcParams['figure.figsize'] = [33, 18 if is_noiso else 20]
    plt.rcParams['figure.constrained_layout.use'] = True
    ...
    if Region.startswith('mesh'):
        ...
        all_plots = [
            ...
            in range(len(models))]
    inputs_by_plotID = inputs
    outputs_by_plotID = outputs
    ...
    uppers_by_plotID = [None] * len(all_plots) 
    uppers_xData_by_plotID = [None] * len(all_plots)
    ...
    for i in range(len(all_uppers)):
        ...
        for term in range(terms):
            for plot_id in range(len(all_plots)):
                ...
                # Get the current title object
                ax = plt.gca()
                title_obj = ax.title

                # Get the current title text
                current_title = title_obj.get_text()

                # Set the title again, but with bold fontweight
                ax.set_title(current_title, fontweight="bold")

                uppers_xData_by_plotID[plot_id] = all_plots[plot_id][2 * term + 1][0].get_xdata()
                uppers_by_plotID[plot_id] = all_uppers[i][-1][plot_id]
        ...
        # for j in range(5):
        def calculateR2(measured_stretch, measured_stress, pred_stretch_raw, pred_stress_raw):
            try:
                pred_stretch_raw = np.array(np.array(pred_stretch_raw))
                pred_stress_raw = np.squeeze(np.array(pred_stress_raw))
                f = interp1d(pred_stretch_raw, pred_stress_raw, kind='linear', fill_value="extrapolate", bounds_error=False) # linear interpolation.
                pred_stress = f(measured_stretch)
                R2 = r2_score(measured_stress, pred_stress)
                return np.round(max(R2, 0), 4)
            except:
                return -1
        ...
        for plot_id in range(len(axes)):
            if (steps[i] + 1) in first_epoch_per_lalpha:
                ...
                r2 = calculateR2(inputs_by_plotID[plot_id], outputs_by_plotID[plot_id],uppers_xData_by_plotID[plot_id], uppers_by_plotID[plot_id])
                axes[plot_id].get_shared_y_axes().get_siblings(axes[plot_id])[0].set_xlabel(f"$R^2$ = {r2:.4f}",labelpad=-50)
            else:
...
def plotMap(ax, Psi_model, model_weights, model, terms, input, output, direction_string, title, num_points=17):
    ...
    if terms < len(cmaplist):
        cmaplist = cmaplist[len(cmaplist)-terms:]
    all_plots, predictions = color_map(ax, input, model, model_weights, Psi_model, cmaplist, terms, return_predictions=True)
    ...
    # scatter_handle = ax.scatter(input, output, s=300, zorder=25, lw=3, facecolors='red', edgecolors='k', clip_on=False)
...
def plot_raw_data():
    ...
    ylabels = ["X stress [kPa]", "Y stress [kPa]", "x stress [kPa]"]
    xlabels = ["X stretch [-]", "Y stretch [-]", "x stretch [-]"]
    n_spaces = 55
    titles = [["X", "Y", "Biaxial", "N/A", "N/A"], [""] * 5,
             [" ", "", "", "", ""]]
