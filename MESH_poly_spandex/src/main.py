...
# Define relevant flags and training parameters
p = 0.5 # Set to 1 to L1 regularization instead of l0.5 regularization
alphas = [0] # List of regularization parameters (must start with 0)
modelFit_mode_all = ['012']  # List of which data is used for training (all data, 0-90 orientation only, 45-135 orientation only)
Region = 'mesh' # Material type (specifies which spreadsheet of raw data to load)
cann_names = ["cann_I4ws"] # List of names of folders to save results in
models = [ortho_cann_2ff] # List of models to train with
models_2term = [ortho_cann_2ff_2term] # List of models that can take as a parameter which parameters to restrict
is_I4betas = [False]
...
if __name__ == "__main__":
    ...
    # plot_arch_comp_graph([os.path.join(get_path(Region, '012'), cann_name, str(0)) for cann_name in cann_names[0:2]])
